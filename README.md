...
- **调试截图工具**：附带 `debug_capture.py`，一键截屏/框选保存测试图，方便收集各种牌面样本。
...
- **模板目录**：源码版为 `templates/`，打包版为 `_internal/templates/`。在线修正会直接更新其中对应的牌面图片，保存前自动做降噪与背景白化处理，模板越用越干净。

---

## 🔨 调试截图工具

`debug_capture.py` 用于快速收集各种牌面的测试截图，保存到 `input/` 目录，文件名按已有编号自动顺延（如已有 `1.png` 则保存为 `2.png`），方便批量导入自测：
...
> 不传参数时自动截取鼠标所在屏幕（F2、F3 均如此）；也可传 1、2、3… 按 mss 显示器索引指定屏幕（启动时会列出各屏幕的索引）。收集到的图片可直接用于识别自测或模板训练。

---

## ❓ 常见问题

- **热键没反应**：请确认程序已在运行且未被最小化到托盘；若游戏以管理员身份运行，需同样以管理员身份启动本程序；若被杀毒软件拦截，请将程序加入白名单。
...
debug_capture.py     调试截图工具：F2 全屏 / F3 框选保存到 input/
